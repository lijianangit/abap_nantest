class zcl_z250226_dpc_ext definition
  public
  inheriting from zcl_z250226_dpc
  create public .
...
class zcl_z250226_dpc_ext implementation.
  ...
  method ztab1set_create_entity.
    data ls_requst_input_data type zcl_z250226_mpc=>ts_ztab1.

    io_data_provider->read_entry_data( importing es_data = ls_requst_input_data ).
    data(lo_uuid) = cl_uuid_factory=>create_system_uuid( ).
    try.
        data(lv_uuid_c32) = lo_uuid->create_uuid_c32( ).
      catch cx_uuid_error.
        "handle exception
    endtry.
    ls_requst_input_data-uuid = lv_uuid_c32.
